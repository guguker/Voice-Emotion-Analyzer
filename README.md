...
- Текущая оконная обработка рассчитана на записи длиннее двух секунд. Неполный остаток не анализируется; если последнее полное окно заканчивается точно на конце сигнала, оно также пропускается.
- При записи с микрофона тихие блоки отбрасываются, а остальные склеиваются. Временная шкала относится к этому сокращённому сигналу и не сохраняет длительность исходных пауз.
